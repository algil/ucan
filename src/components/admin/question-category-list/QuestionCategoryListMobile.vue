<template>
  <v-list two-line>
    <template v-for="(questionCategory, index) in questionCategories">
      <question-category-list-mobile-item :question-category="questionCategory" @onSelect="onSelect" :key="index"></question-category-list-mobile-item>
      <v-divider v-show="index !== questionCategories.length - 1" :key="index"></v-divider>
    </template>
  </v-list>
</template>
